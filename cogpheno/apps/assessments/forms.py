...
class BehaviorForm(ModelForm):
    ...
    synset = forms.ChoiceField(choices = [],label="synset")
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Get all synset options
        synsets = Word(self.instance.name).get_synsets()
        choices = []
        for s in synsets:
            choices.append([s.name(),s.definition()])
        choices.append(["None","None"])
        self.fields['synset'].choices = choices
